        ...
        try:
            ...
            if trigger_ai:
                ...
                if current_state.next:
                    logger.info(" [HITL] Graph is paused. Resume...")
                ...
                if post_state.next:
                    ...
                    if tasks and tasks[0].interrupts:
                        ...
                        svc = interrupt_data.get("service_name", "Service")
                        prc = interrupt_data.get("price", "Not clear yet")
                        
                        
                        hitl_msg = f"\n\n **CONFIRMATION SYSTEM:** Do you agree to proceed with booking the service and generating the Payment Form for **{svc}** (Price: {prc})? *(Type 'Agree' or 'Cancel')*"
                        full_text += hitl_msg
        ...
        while True:
            ...
            if trigger_ai:
                ...
                try:
                    ...
                    if current_state.next:
                        logger.info(f"[HITL Websocket] Room {room_id} Awaiting approval. Checking feedback...")
                        is_approved = any(word in text_upper for word in ["YES", "OK", "AGREE", "BOOK", "THANH TOÁN", "ĐỒNG Ý", "TẠO"])
                        from langgraph.types import Command
                        input_data = Command(resume={"approved": is_approved})
                    else:
                    ...
                    if post_state.next:
                        ...
                        if tasks and tasks[0].interrupts:
                            ...
                            svc = interrupt_val.get("service_name", "service")
                            prc = interrupt_val.get("price", "negotiable price")
                            full_text += f"\n\n **CONFIRMATION FROM ADMIN/USER:** I have prepared the schedule for **{svc}** ({prc}). Do you all agree to issue a payment invoice? (Type 'Agree' to continue)"
